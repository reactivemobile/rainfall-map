...
class RainfallRepository @Inject constructor(
    ...
    suspend fun getStationDetails(station: Station): Station =
        try {
            withContext(ioDispatcher) {
                val response: StationDetailsDTO = rainfallClient.getStationDetails(station.id)
                apiMapper.mapStationDetailsDtoToDomainObject(station, response.items.firstOrNull())
            }
        } catch (exception: Exception) {
            exception.printStackTrace()
            station
